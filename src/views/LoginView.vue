<template>
  <div id="container">
    ...
    <div class="textCenter NanumSquareRound" style="margin: 0 0 10px 0">
      아직 회원이 아니신가요?
      <router-link to="/register">Sign up</router-link> | 비밀번호를 잊으셨나요?
    </div>
    <div
      id="mainLogo"
      class="textCenter"
...
export default {
        ...
        .then(({ data }) => {
          if (data == "0") {
            ...
            if (this.rememberMe == true) {
              ...
              document.cookie =
                "pw=" + this.pw + ";expires=" + date.toUTCString() + ";path=/";
              document.cookie =
              ...
              document.cookie =
                "pw= " + "; expires=" + new Date().toUTCString() + "; path=/";
              document.cookie =
  ...
  created() {
    window.Kakao.init("7421f111a6cf185511db9211279df5aa"); //카카오 key로 초기화
    try {
      if (document.cookie.split("; ")[2].split("=")[1]) {
        this.rememberMe = true;
        console.log(document.cookie.split("; ")[0].split("=")[1]);
        console.log(document.cookie.split("; ")[1].split("=")[1]);
        this.id = document.cookie.split("; ")[0].split("=")[1];
        this.pw = document.cookie.split("; ")[1].split("=")[1];
      }
    } catch (e) {
      console.log("쿠키 기록없음");
    }
